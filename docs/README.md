...
Diese Datei ist die ausführliche Betriebsdokumentation für die Ausführung und Erweiterung. Die
formale Dokumentation von Daten, Code, Datenschutz und Ablage steht in der
[Root-README](../README.md). Das Release enthält den Code, Tests,
Konfigurationen und Dokumentation. Der lokale Ordner `DicomData/` mit den
Eingabedokumenten sowie die ScrabbleGAN-Quell- und Modellartefakte werden aus
Gründen von Größe, Rechten und Datenschutz nicht mitgeliefert und müssen für
bestimmte Läufe separat bereitgestellt werden.
...
- Die Voraussetzungen und Betriebsgrenzen der Handschriftverarbeitung stehen
  in [`tools/handwriting/README.md`](../tools/handwriting/README.md) und
  [`tools/handwriting/scrabblegan/README.md`](../tools/handwriting/scrabblegan/README.md).
- Die manuelle Funktionsprüfung ist in
  [`tools/visual_checks/README.md`](../tools/visual_checks/README.md)
  beschrieben. Ergebnis- und Validierungswerkzeuge stehen unter
  [`tools/thesis_results/README.md`](../tools/thesis_results/README.md).
...
| matplotlib | Vorschaubilder und Visualisierungen |
| Faker | Erzeugung synthetischer Identitätswerte |
| Docker | Isolierte Legacy-Runtime für ScrabbleGAN-Handschrift |
...
|-- tools/thesis_results/         # Ergebnis- und Validierungswerkzeuge
|-- tools/visual_checks/          # Manuelle Funktions- und Sichtprüfungen
|-- configs/
...
|   |-- integration/
|   `-- unit/
|-- DicomData/                    # separat bereitzustellen, nicht versioniert
|-- thesis-results/               # erzeugte Evaluationsdaten, nicht versioniert
...
Aus dem Stammverzeichnis `injection-pipeline-release/` ausführen. Benötigt
werden Python `>= 3.13` und `uv`.

```bash
cd injection-pipeline-release
uv sync --extra dev
```

Nach dem Kopieren oder Entpacken des Release-Ordners beziehungsweise in einem
neuen Virtual Environment `uv sync --extra dev` ausführen. Das Dev-Extra
installiert `pytest`, `ruff` und `mypy`.

## Voraussetzungen für die Ausführung

### Lokale Eingabedaten

Der Release-Ordner enthält absichtlich keine medizinischen Eingabedaten. Für
normale DICOM-/JPG-Läufe muss im Projektstamm ein separat bereitgestellter
Ordner `DicomData/` liegen. Die Minimalstruktur lautet:

```text
DicomData/
|-- Dicom-Files/       # mindestens eine unterstützte .dcm-Datei
|-- images/            # mindestens eine .jpg- oder .jpeg-Datei
|-- PDF/               # PDF-Vorlagen für PDF- und Visual-Checks
`-- HandwritingAssets/ # nur für Handschriftläufe
```

Die DICOM- und JPG-Dateien müssen bereits anonymisiert sein. Für die
vollständige manuelle Funktionssuite werden mindestens fünf unterstützte
DICOM-Dateien, vier JPG/JPEG-Dateien und eine PDF-Vorlage benötigt. Ein
einzelner normaler Lauf kann stattdessen mit `--input` auf genau eine
vorhandene DICOM- oder JPG-Datei zeigen. Die Unit- und viele Integrationstests
erzeugen ihre synthetischen Fixtures selbst und benötigen dafür keinen
`DicomData/`-Ordner.

### ScrabbleGAN-Handschrift

Die normale Textinjektion mit `arial`, `calibri`, `tahoma` oder `consolas`
benötigt keine ScrabbleGAN-Dateien. Für `--font-family handwriting`,
`generate-handwriting` und Handschriftprüfungen sind jedoch alle folgenden
...
|-- scrabblegan/
    |-- checkpoints/
    |   |-- latest_net_G.pth       # erforderlicher Generator-Checkpoint
    |   `-- test_opt.txt           # alternativ train_opt.txt/options.json
    |-- source/                    # offizieller ScrabbleGAN-Source-Checkout
    |   `-- .git_commit             # erforderlich, falls kein .git vorhanden ist
    `-- runs/                      # erzeugte Assets und Manifeste
```

Der Checkpoint, der Options-Sidecar und der Source-Commit müssen zueinander
passen. Der Checkpoint-Hash wird automatisch berechnet, wenn kein Hash über
`--handwriting-checkpoint-sha256` übergeben wird. Ein vorhandener
`latest_net_D.pth`- oder `latest_net_OCR.pth`-Begleitcheckpoint kann neben dem
Generator-Checkpoint abgelegt werden und wird vom Wrapper berücksichtigt.
Die Pipeline lädt keine Modellgewichte automatisch herunter. Source-Checkout,
Checkpoint, Sidecar und erzeugte Handschrift-Assets bleiben lokale, nicht
versionierte Dateien.

Für die isolierte Legacy-Runtime muss Docker Desktop beziehungsweise Docker
verfügbar sein. Das Image einmalig aus dem Projektstamm bauen:

```bash
docker build --platform linux/amd64 -t injection-scrabblegan tools/handwriting/scrabblegan
```

Die Runtime verwendet Python 3.6 und PyTorch 1.2 innerhalb des Containers und
bleibt dadurch von der Python-3.13-Hauptumgebung getrennt. Auf Apple-Silicon-
und anderen ARM-Hosts ist `linux/amd64`-Emulation erforderlich. Für Image,
Build-Cache, Checkpoint und erzeugte Assets sollten mehrere Gigabyte freier
Speicher eingeplant werden. Vollständige Details einschließlich der Ablage
des Upstream-Source-Checkouts stehen in
[`tools/handwriting/scrabblegan/README.md`](../tools/handwriting/scrabblegan/README.md).

## Befehle

Alle Befehle werden aus dem Projektstamm ausgeführt. Ein normaler Lauf ohne
`--input` wählt eine Datei aus `DicomData/Dicom-Files` oder
`DicomData/images`. Für eine eindeutige Wiederholung sollte der Eingabepfad
...
Ein einzelner DICOM-Lauf mit expliziter Eingabe:

```bash
uv run injection-pipeline \
  --input DicomData/Dicom-Files/<datei>.dcm \
  --seed 42 \
  --rotation-angle 20 \
  --run-timestamp 2026-09-20T12:00:00
```

Ein einzelner JPG-Lauf verwendet dieselbe CLI mit einer `.jpg`- oder
`.jpeg`-Datei. Die Ausgaben liegen anschließend unter
`output/<run-id>/`. Ein Lauf erzeugt insbesondere das injizierte Dokument,
`ground_truth.json`, `preview.png`, `preview_annotated.png` und
`run_manifest.json`.
...
uv run injection-pipeline inject-pdf \
  --input-pdf DicomData/PDF/<vorlage>.pdf \
  --input-dicom output/<run-id>/<source-stem>_injected.dcm \
  --dicom-annotation output/<run-id>/ground_truth.json \
