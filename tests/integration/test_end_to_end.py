...
def test_failed_run_directory_is_cleaned_up_for_retry(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    input_dir = Path("fixtures")
    input_dir.mkdir(exist_ok=True)
    input_path = write_synthetic_jpg(input_dir / "synthetic.jpg")
    output_root = Path("output")
    args = Namespace(
        input=str(input_path),
        output_dir=str(output_root),
    ...
    original_write_run_artifacts = runner.write_run_artifacts
    first_call = True

    def fail_once(record: Any, paths: dict[str, Path]) -> None:
        nonlocal first_call
        if first_call:
            first_call = False
            raise RuntimeError("simulated artifact write failure")
        original_write_run_artifacts(record, paths)

    monkeypatch.setattr(runner, "write_run_artifacts", fail_once)
