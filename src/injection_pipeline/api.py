...
def make_pdf(
    ...
    *,
    ...
    """Compose injected images and native text overlays into a PDF.

    Args:
        images: Image inputs with their required image-space annotations.
        texts: PDF-native text inputs with their annotation metadata.
        pdf: Source PDF template. It is never modified.
        output_dir: Directory for the clean PDF, annotated PDF, and JSON
            annotation sidecar.
        seed: Optional deterministic seed for layout and image rotation.

    Returns:
        Paths and metadata for the generated PDF artifacts.

    Raises:
        ValueError: If inputs, paths, annotations, or the PDF template are
            invalid, or if an output would alias an input.
    """
...
def inject_function(
    ...
    *,
    ...
    """Run one controlled synthetic-PII injection for a DICOM or JPG input.

    The visible text is composed as ``prefix + value + suffix``. The pipeline
    writes a complete run bundle below its configured output directory and
    returns the paths to the injected document and its ground-truth JSON.

    Args:
        category: Category label stored in the ground-truth annotations.
        value: Synthetic value to inject.
        prefix: Visible text placed before ``value``.
        suffix: Visible text placed after ``value``.
        handwritten: Whether to use the isolated handwriting renderer.
        documentType: Input type, either ``"dcm"`` or ``"jpg"``.
        output_dir: Optional directory receiving copies of the two returned
            artifacts.
        handwriting_ink_color: Handwriting ink mode: ``auto``, ``black``,
            ``gray``, or ``white``.
        handwriting_contrast_mode: Handwriting contrast mode, either
            ``none`` or ``halo``.
        seed: Optional deterministic seed for identity and layout choices.
        input_path: Optional explicit DICOM/JPG input path.
        rotation_degrees: Optional rotation from the supported angle set.
        run_timestamp: Optional timestamp used to construct a reproducible run
            identifier.
        font_size_pct: Relative font size, where ``100`` is the default.
        placement_mode: Placement strategy: ``corners``, ``free``, or
            ``center``.

    Returns:
        A tuple containing the injected document path and ground-truth JSON
        path.

    Raises:
        ValueError: If an argument, input path, document type, or output
